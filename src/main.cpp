...
const uint8_t ARDUINO_RX = D4;    // connect to TX of MP3 Player module
const uint8_t ARDUINO_TX = D5;    // connect to RX of MP3 Player module
...
const uint8_t PIN_SWITCH = D6;    // play/pause toggle digital pin, active low (PULLUP)
const uint8_t PIN_LED = D7;       // LED to show status
...
void loop()
{
  int rtrn = mp3.check();        // run the mp3 receiver
  //PRINTS(rtrn);
